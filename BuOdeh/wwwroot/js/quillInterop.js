window.quillInterop = {
    ...
    initializeQuill: function (elementId, initialContent) {
        ...
        quill.on('text-change', () => {            
            DotNet.invokeMethodAsync("BuOdeh", "UpdateQuillContentApart", elementId, quill.root.innerHTML);
        });

        // Event listener to update the Blazor component when content changes
        quill.on('text-change', () => {
            DotNet.invokeMethodAsync("BuOdeh", "UpdateQuillContentPengumuman", elementId, quill.root.innerHTML);
        });
    },
